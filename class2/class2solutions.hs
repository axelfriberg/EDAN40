...
<interactive>:1:3:
    Couldn't match expected type â€˜[a]â€™
                with actual type â€˜(b0 -> c0) -> (a0 -> b0) -> a0 -> c0â€™
    Probable cause: â€˜(.)â€™ is applied to too few arguments
    In the second argument of â€˜(:)â€™, namely â€˜(.)â€™
    In the expression: (: (.))
Prelude> :t ((.):)
((.):)
  :: [(b -> c) -> (a -> b) -> a -> c]
